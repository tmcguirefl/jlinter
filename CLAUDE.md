...
Provide a **headless** J lint tool so agents can check and correct generated J without Qt. Core analysis is the stock addon `debug/lint`; this project is a report/CLI wrapper packaged as JAL addon **`tmcguire/jlinter`**.
...
- `FOLDER=: 'tmcguire/jlinter'` (lowercase, no spaces)
...
| Path | Role |
|------|------|
| `manifest.ijs` | JAL/Pacman manifest (`FOLDER` tmcguire/jlinter) |
| `jlinter.ijs` | entry for `load 'tmcguire/jlinter'` |
